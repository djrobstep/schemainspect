...
def test_postgres_objects():
    ...
    assert ex.create_statement == 'create extension if not exists "name" with schema "schema" version \'1.2\';'
    assert ex.update_statement == 'alter extension "schema"."name" update to version \'1.2\';'
    ex2 = deepcopy(ex)
    assert ex == ex2
    ex2.version = '2.1'
    assert ex != ex2
    ...
    assert c.create_statement == 'alter table "public"."films" add constraint "firstkey" PRIMARY KEY using index "firstkey";'
    c2 = deepcopy(c)
    assert c == c2
    c.index = None
    assert c != c2
